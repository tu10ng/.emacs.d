...
(setq awesome-tray-active-modules       ;flycheck total,
      '("location" "belong" "mode-name" "buffer-name" "file-path"
...
(defun tu10ng/speed-type-buffer-n (n)
  "type first N chars in current buffer."
  (interactive "P")
  (if n
      (speed-type--setup (buffer-substring-no-properties (point-min) n))
    (speed-type-buffer 1)))
...
Call `tu10ng/speed-type-buffer-n' to set a new buffer for typing."
  ...
  (let* ((char (if char char 1000))
    ...
    (print (concat "fish -c 'fd . " path " --extension " ext " | xargs cat > " tu10ng/typegame-tmpfile "'"))
    ;; failure
    (if (f-empty? tu10ng/typegame-tmpfile)
      ...
      (replace-string "  " "")
      (goto-char 0)    
      (flush-lines "//")                ;execute from current point, need reset point to act on whole buffer
      (save-buffer)
      (tu10ng/speed-type-buffer-n (if (> (point-max) char) char (point-max)))
      (kill-buffer "typegame"))))

(tu10ng/typegame 1000 "~/st" "c")
